...
<rect width="151" height="15" fill="white"/>
  ...
  const LOGO_SVG_WHITE = LOGO_SVG.replace(/currentColor/g, '#ffffff');
    ...
    return `
        ...
        <div class="x-nav-primary">
          <div class="x-nav-item" data-priority="high">
            <button class="x-nav-link" aria-expanded="false" data-dropdown="directory">
              Find a Creative<span class="x-nav-chevron">${icons.chevronDown}</span>
            </button>
      ...
      <div class="x-search-overlay" id="x-search-overlay">
        <div class="x-search-overlay-header">
          <div class="x-search-overlay-logo">${LOGO_SVG_WHITE}</div>
          <button class="x-search-overlay-close" data-action="close-search">${icons.x}</button>
        </div>
  ...
  function setupEventListeners(header) {
    ...
    const hamburger = header.querySelector('[data-action="menu"]');
    const searchBtn = header.querySelector('[data-action="search"]');
    const navOverlay = header.querySelector('#x-nav-overlay');
    const searchOverlay = header.querySelector('#x-search-overlay');
    const closeMenuBtn = header.querySelector('[data-action="close-menu"]');
    const closeSearchBtn = header.querySelector('[data-action="close-search"]');
    const findCreativeToggle = header.querySelector('[data-toggle="find-creative"]');
    const findCreativeSubnav = header.querySelector('#subnav-find-creative');
    ...
    searchBtn?.addEventListener('click', () => {
      ...
      document.body.classList.add('nav-overlay-open');
      setTimeout(() => searchOverlay?.querySelector('.x-search-input')?.focus(), 100);
    });
    ...
    // Find a Creative toggle in overlay
    findCreativeToggle?.addEventListener('click', () => {
      const isOpen = findCreativeSubnav?.classList.toggle('open');
      findCreativeToggle.classList.toggle('active', isOpen);
    });

    // Close on outside click
    document.addEventListener('click', (e) => {
      if (directoryDropdown && !e.target.closest('[data-dropdown="directory"]') && !e.target.closest('#dropdown-directory')) {
        directoryDropdown.classList.remove('open');
        directoryBtn?.setAttribute('aria-expanded', 'false');
      }
  ...
  async function init() {
    ...
    const checkMemberstack = () => new Promise((resolve) => {
      if (window.$memberstackDom) return resolve(window.$memberstackDom);
      let attempts = 0;
      const check = setInterval(() => {
        attempts++;
        if (window.$memberstackDom) { clearInterval(check); resolve(window.$memberstackDom); }
        else if (attempts >= 30) { clearInterval(check); resolve(null); }
      }, 100);
    });
  ...
  window.MTNSNav = { init, render: renderNav, styles };
