...
MTNS MADE Team`}async function M(){const[e,t,i,o,r,a,n,s,d,l,p,c]=await Promise.all([F(),H(),V(),J(),Y(),G(),K(),Q(),X(),W(),Z(),B()]);return T=c,{recentMembers:e,memberStats:t,incompleteProfiles:i,failedSignups:o,recentEvents:r,eventStats:a,recentOpportunities:n,opportunityStats:s,recentProjects:d,messageStats:l,recentActivity:p,membershipTypes:c,loadedAt:new Date}}async function F(){const{data:e,error:t}=await m.from("members").select(`
...
This will remove it from the site. This cannot be undone.`)){a.disabled=!0,a.textContent="Deleting...";try{const l=await(await fetch(`${v}/functions/v1/admin-tools`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"delete-opportunity",opportunityId:n})})).json();l.success?(alert(`"${s}" has been deleted.`),g(e)):(alert(`Failed to delete: ${l.error}`),a.disabled=!1,a.textContent="Delete")}catch(d){console.error("Delete opportunity error:",d),alert("Error deleting opportunity. Please try again."),a.disabled=!1,a.textContent="Delete"}}})})}function oe(e){const t=[];e.memberStats.pendingSync>0&&t.push({type:"warning",text:"Members pending Webflow sync",count:e.memberStats.pendingSync});const i=e.incompleteProfiles.filter(o=>(Date.now()-new Date(o.created_at))/864e5>7&&!o.profile_reminder_sent_at);return i.length>0&&t.push({type:"info",text:"Incomplete profiles (7+ days, no reminder sent)",count:i.length}),e.eventStats.pending>0&&t.push({type:"info",text:"Events pending review",count:e.eventStats.pending}),e.opportunityStats.pending>0&&t.push({type:"info",text:"Opportunities pending review",count:e.opportunityStats.pending}),e.memberStats.lapsed>0&&t.push({type:"error",text:"Lapsed subscriptions",count:e.memberStats.lapsed}),t.length===0?`
    ...
    `,p}async function w(){const e=document.getElementById("support-tracker-root");if(!e)return;e.innerHTML='<div style="padding:20px;color:#888;font-size:13px;">Loading...</div>';const t=await be();ge(e,t)}async function ve(){const e=document.getElementById("sop-root");if(!e)return;e.innerHTML='<div style="padding:20px;color:#888;font-size:13px;">Loading...</div>';const{data:t,error:i}=await m.from("sops").select("*").order("category",{ascending:!0}).order("title",{ascending:!0});if(i){e.innerHTML=`<div class="empty-state" style="padding:40px 20px;">Error loading SOPs: ${u(i.message)}</div>`;return}if(!t||t.length===0){e.innerHTML='<div class="empty-state" style="padding:40px 20px;">No SOPs yet.</div>';return}e.innerHTML=t.map(o=>`
    ...
    `).join("")}function ge(e,t){var o,r;const i=j==="archive";if(e.innerHTML=`
