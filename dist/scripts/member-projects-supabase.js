(function(){console.log("Member projects Supabase script loaded");const P="https://epszwomtxkpjegbjbixr.supabase.co",j="sb_publishable_567NLTP3qU8_ONMFs44eow_WoNrIlCH",z="project-images",w=20,V={emerging:2,professional:5,"small-business":5,"not-for-profit":5,"large-business":8,"spaces-suppliers":5};let y=null,v=null,f=[],x={directories:[],subDirectories:[]},q=!1;const W=`
  ...
  `;function J(){return window.supabase?(y=window.supabase.createClient(P,j),console.log("Supabase client initialized"),!0):(console.error("Supabase JS library not loaded"),!1)}function I(r){if(!r)return 2;const e=r.toLowerCase();return V[e]||2}function X(r){return r.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"").substring(0,50)+"-"+Date.now().toString(36)}function _(r){if(!r||r.trim()==="")return"";let e=r.trim();/^https?:\/\//i.test(e)||(e="https://"+e);try{return new URL(e),e}catch{return""}}function k(r){if(!r||r.trim()==="")return!0;let e=r.trim();/^https?:\/\//i.test(e)||(e="https://"+e);try{const o=new URL(e);if(!o.hostname||!o.hostname.includes("."))return!1;const t=o.hostname.split(".");return!(t[t.length-1].length<2)}catch{return!1}}function L(r){if(!r||r.trim()==="")return!0;let e=r.trim();/^https?:\/\//i.test(e)||(e="https://"+e);try{const t=new URL(e).hostname.toLowerCase();return!!(t.includes("youtube.com")||t.includes("youtu.be")||t.includes("vimeo.com"))}catch{return!1}}async function K(){try{const{data:r,error:e}=await y.from("directories").select("id, webflow_id, name, slug").order("display_order");if(e)throw e;const{data:o,error:t}=await y.from("sub_directories").select("id, webflow_id, name, slug, directory_slug").order("name");if(t)throw t;return console.log(`Loaded ${r.length} directories and ${o.length} sub-directories`),{directories:r,subDirectories:o}}catch(r){return console.error("Error loading categories:",r),{directories:[],subDirectories:[]}}}async function Z(r){try{const{data:e,error:o}=await y.from("projects").select(`
          *,
          project_sub_directories (
            sub_directory_id
          )
        `).eq("memberstack_id",r).eq("is_deleted",!1).order("display_order",{ascending:!0});if(o)throw o;const t=e.map(i=>{var l;return{...i,categories:((l=i.project_sub_directories)==null?void 0:l.map(p=>p.sub_directory_id))||[],gallery_images:i.gallery_images||[]}});return console.log(`Loaded ${t.length} projects for member ${r}`),t}catch(e){return console.error("Error loading projects:",e),[]}}async function Q(r){try{const{categories:e,...o}=r,{data:t,error:i}=await y.from("projects").insert({memberstack_id:v.id,name:o.name,slug:X(o.name),description:o.description,feature_image_url:o.feature_image_url||null,gallery_images:o.gallery_images||[],external_link:o.external_link||null,showreel_link:o.showreel_link||null,display_order:o.display_order||0,is_draft:!1,is_deleted:!1}).select().single();if(i)throw i;if(e&&e.length>0){const l=e.map(s=>({project_id:t.id,sub_directory_id:s})),{error:p}=await y.from("project_sub_directories").insert(l);p&&console.error("Error linking categories:",p)}return console.log("Project created:",t.id),{...t,categories:e||[],gallery_images:t.gallery_images||[]}}catch(e){throw console.error("Error creating project:",e),e}}async function ee(r,e){try{const{categories:o,...t}=e,{data:i,error:l}=await y.from("projects").update({name:t.name,description:t.description,feature_image_url:t.feature_image_url||null,gallery_images:t.gallery_images||[],external_link:t.external_link||null,showreel_link:t.showreel_link||null,display_order:t.display_order||0}).eq("id",r).eq("memberstack_id",v.id).select().single();if(l)throw l;const{error:p}=await y.from("project_sub_directories").delete().eq("project_id",r);if(p&&console.error("Error deleting old categories:",p),o&&o.length>0){const s=o.map(d=>({project_id:r,sub_directory_id:d})),{error:a}=await y.from("project_sub_directories").insert(s);a&&console.error("Error linking categories:",a)}return console.log("Project updated:",r),{...i,categories:o||[],gallery_images:i.gallery_images||[]}}catch(o){throw console.error("Error updating project:",o),o}}async function re(r){try{const{error:e}=await y.from("projects").update({is_deleted:!0}).eq("id",r).eq("memberstack_id",v.id);if(e)throw e;return console.log("Project deleted:",r),!0}catch(e){throw console.error("Error deleting project:",e),e}}async function $(r,e=null){try{const o={memberstack_id:v.id,activity_type:r};e&&(o.entity_type="project",o.entity_id=e.id||null,o.entity_name=e.name||null),await fetch(`${P}/functions/v1/log-activity`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${j}`,apikey:j},body:JSON.stringify(o)})}catch(o){console.warn("Failed to log activity:",o)}}const M=3*1024*1024,S=2e3;async function te(r){const e=(r.size/1024/1024).toFixed(1);return r.size<=M?(console.log(`Image ${r.name} is ${e}MB - no compression needed`),r):(console.log(`Compressing image ${r.name} from ${e}MB...`),new Promise((o,t)=>{const i=new Image,l=document.createElement("canvas"),p=l.getContext("2d");i.onload=()=>{let{width:s,height:a}=i;if(s>S||a>S){const m=Math.min(S/s,S/a);s=Math.round(s*m),a=Math.round(a*m)}l.width=s,l.height=a,p.drawImage(i,0,0,s,a);const d=m=>{l.toBlob(c=>{if(!c){t(new Error("Failed to compress image"));return}const g=(c.size/1024/1024).toFixed(1);if(console.log(`Compressed to ${g}MB at quality ${m}`),c.size<=M||m<=.3){const n=new File([c],r.name,{type:"image/jpeg",lastModified:Date.now()});o(n)}else d(m-.1)},"image/jpeg",m)};d(.8)},i.onerror=()=>t(new Error("Failed to load image for compression")),i.src=URL.createObjectURL(r)}))}async function A(r,e){const o=await te(r),t=Date.now(),i=o.type==="image/jpeg"?"jpg":r.name.split(".").pop(),l=`${v.id}/${e||"new"}/${t}.${i}`,{data:p,error:s}=await y.storage.from(z).upload(l,o,{cacheControl:"3600",upsert:!1});if(s)throw s;const{data:{publicUrl:a}}=y.storage.from(z).getPublicUrl(l);return console.log("Image uploaded:",a),a}function U(r){const e=x.subDirectories.find(o=>o.id===r);return e?e.name:r}function oe(r){return x.subDirectories.filter(e=>e.directory_slug===r)}function T(r){const e=r.querySelector("#mp-form-external_link"),o=r.querySelector("#mp-url-error");if(!e||!o)return;const t=()=>{const i=e.value.trim();i===""?(e.classList.remove("error","valid"),o.classList.remove("visible")):k(i)?(e.classList.remove("error"),e.classList.add("valid"),o.classList.remove("visible")):(e.classList.remove("valid"),e.classList.add("error"),o.classList.add("visible"))};e.addEventListener("blur",t),e.addEventListener("input",()=>{e.classList.contains("error")&&k(e.value)&&(e.classList.remove("error"),e.classList.add("valid"),o.classList.remove("visible"))}),e.value.trim()&&t()}function N(r){const e=r.querySelector("#mp-form-showreel_link"),o=r.querySelector("#mp-showreel-error");if(!e||!o)return;const t=()=>{const i=e.value.trim();i===""?(e.classList.remove("error","valid"),o.classList.remove("visible")):L(i)?(e.classList.remove("error"),e.classList.add("valid"),o.classList.remove("visible")):(e.classList.remove("valid"),e.classList.add("error"),o.classList.add("visible"))};e.addEventListener("blur",t),e.addEventListener("input",()=>{e.classList.contains("error")&&L(e.value)&&(e.classList.remove("error"),e.classList.add("valid"),o.classList.remove("visible"))}),e.value.trim()&&t()}const F=[{text:"Preparing project...",progress:10},{text:"Optimising images...",progress:25},{text:"Compiling data...",progress:45},{text:"Populating categories...",progress:60},{text:"Syncing to portfolio...",progress:80},{text:"Finalising...",progress:95}];function ie(r="Creating Project"){const e=document.createElement("div");return e.className="mp-progress-overlay",e.id="mp-progress-overlay",e.innerHTML=`
    ...
    `,document.body.appendChild(e),e}function ae(r,e){if(!r)return;const o=F[e];if(!o)return;const t=r.querySelector(".mp-progress-bar-fill"),i=r.querySelector(".mp-progress-status");t&&(t.style.width=o.progress+"%"),i&&(i.classList.add("fade"),setTimeout(()=>{i.textContent=o.text,i.classList.remove("fade")},150))}function se(){const r=document.getElementById("mp-progress-overlay");if(r){const e=r.querySelector(".mp-progress-bar-fill"),o=r.querySelector(".mp-progress-status");e&&(e.style.width="100%"),o&&(o.textContent="Complete!"),setTimeout(()=>{r.remove()},500)}}async function R(r,e="Creating Project"){const o=ie(e);let t=0;const i=setInterval(()=>{t<F.length&&(ae(o,t),t++)},600);try{const l=await r();return clearInterval(i),se(),l}catch(l){clearInterval(i);const p=document.getElementById("mp-progress-overlay");throw p&&p.remove(),l}}function E(r){var p;if(!q){r.innerHTML=`
        <div class="mp-empty">
      ...
      `;return}if(f.length===0){r.innerHTML=`
        <div class="mp-empty">
          <p>You don't have any projects yet</p>
          <button class="mp-btn" id="mp-add-first">Add Your First Project</button>
        </div>
      `,r.querySelector("#mp-add-first").addEventListener("click",()=>G(r));return}const e=(p=v==null?void 0:v.customFields)==null?void 0:p["membership-type"],o=I(e),i=o-f.length<=0;let l=`
      <div class="mp-header">
        <h2>My Projects</h2>
    ...
    `;f.forEach((s,a)=>{l+=le(s)}),l+="</div>",r.innerHTML=l,r.querySelector("#mp-add-project").addEventListener("click",()=>{var d;const s=(d=v==null?void 0:v.customFields)==null?void 0:d["membership-type"],a=I(s);if(f.length>=a){pe(a,s);return}G(r)}),r.querySelectorAll(".mp-project-card").forEach((s,a)=>{const d=f[a];ne(s,d,r)})}function le(r,e){let o="";const t=r.description||"";o+=`
      <div class="mp-field">
    ...
    `;const l=r.feature_image_url,p=r.gallery_images||[];return(l||p.length>0)&&(o+=`
        <div class="mp-field">
    ...
    `}function ne(r,e,o){const t=r.querySelector(".mp-project-content"),i=r.querySelector(".mp-toggle-details"),l=r.querySelector(".mp-toggle-icon"),p=r.querySelector(".mp-edit-btn"),s=r.querySelector(".mp-delete-btn");i.addEventListener("click",a=>{a.stopPropagation();const d=t.classList.toggle("open");l.classList.toggle("open",d)}),p.addEventListener("click",a=>{a.stopPropagation(),ce(e,o)}),s.addEventListener("click",async()=>{if(confirm("Are you sure you want to delete this project?")){s.disabled=!0,s.textContent="Deleting...";try{const a=e.name,d=e.id;await re(d),await $("project_delete",{id:d,name:a}),f=f.filter(m=>m.id!==d),E(o)}catch(a){console.error("Error deleting project:",a),alert("Error deleting project. Please try again."),s.disabled=!1,s.textContent="Delete"}}})}function O(r=[]){let e=`
    ...
    `;return x.directories.forEach(o=>{e+=`<button type="button" class="mp-parent-btn" data-parent="${o.slug}">${o.name}</button>`}),e+="</div>",x.directories.forEach(o=>{const t=oe(o.slug);e+=`<div class="mp-child-categories" data-parent="${o.slug}">`,t.forEach(i=>{const l=r.includes(i.id);e+=`<button type="button" class="mp-child-btn ${l?"selected":""}" data-id="${i.id}">${i.name}</button>`}),e+="</div>"}),e+=`
        <div class="mp-selected-categories" style="${r.length?"":"display: none;"}">
          <h5>Selected Categories</h5>
          <div class="mp-selected-list"></div>
        </div>
      </div>
    `,e}function B(r,e,o){const t=r.querySelectorAll(".mp-parent-btn"),i=r.querySelectorAll(".mp-child-categories"),l=r.querySelector(".mp-selected-list"),p=r.querySelector(".mp-selected-categories");function s(){l.innerHTML=e.map(a=>`<span class="mp-selected-tag">${U(a)}<button type="button" data-id="${a}">&times;</button></span>`).join(""),p.style.display=e.length?"":"none",r.querySelectorAll(".mp-child-btn").forEach(a=>{a.classList.toggle("selected",e.includes(a.dataset.id))})}t.forEach(a=>{a.addEventListener("click",()=>{const d=a.dataset.parent,m=a.classList.contains("active");t.forEach(c=>c.classList.remove("active")),i.forEach(c=>c.classList.remove("visible")),m||(a.classList.add("active"),r.querySelector(`.mp-child-categories[data-parent="${d}"]`).classList.add("visible"))})}),r.querySelectorAll(".mp-child-btn").forEach(a=>{a.addEventListener("click",()=>{const d=a.dataset.id,m=e.indexOf(d);m>-1?e.splice(m,1):e.push(d),s()})}),l.addEventListener("click",a=>{if(a.target.tagName==="BUTTON"){const d=a.target.dataset.id,m=e.indexOf(d);m>-1&&(e.splice(m,1),s())}}),s()}function D(r={}){const e=(r.gallery_images||[]).length;return`
    ...
    `}function H(r,e,o){const t=r.querySelector("#mp-feature-upload"),i=r.querySelector("#mp-feature-input");t.addEventListener("click",l=>{if(l.target.classList.contains("mp-remove-image")){l.stopPropagation(),e.feature_image_url="",t.classList.remove("has-image");const p=t.querySelector("img");p&&p.remove();return}t.classList.contains("has-image")||i.click()}),i.addEventListener("change",async l=>{const p=l.target.files[0];if(p){try{t.style.opacity="0.5";const s=await A(p,e.id);e.feature_image_url=s,t.classList.add("has-image");let a=t.querySelector("img");a||(a=document.createElement("img"),t.insertBefore(a,t.firstChild)),a.src=s,a.alt="Feature",t.style.opacity="1"}catch(s){console.error("Error uploading feature image:",s),alert("Error uploading image. Please try again."),t.style.opacity="1"}i.value=""}}),de(r,e,o)}function de(r,e,o){const t=r.querySelector("#mp-gallery-grid"),i=r.querySelector("#mp-gallery-input"),l=r.querySelector("#mp-gallery-add"),p=r.querySelector("#mp-gallery-count-num");e.gallery_images||(e.gallery_images=[]);function s(){if(t.innerHTML="",e.gallery_images.forEach((c,g)=>{const n=document.createElement("div");n.className="mp-gallery-item has-image",n.dataset.index=g,n.draggable=!0,n.innerHTML=`
    ...
    `,document.body.appendChild(t),t.querySelector("#mp-modal-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",i=>{i.target===t&&t.remove()})}function G(r){const e=[],o={feature_image_url:"",gallery_images:[]},t=document.createElement("div");t.className="mp-modal-overlay",t.innerHTML=`
      <div class="mp-modal">
        <div class="mp-modal-header">
    ...
    `,document.body.appendChild(i),B(i,o),H(i,t,null),T(i),N(i);const l=i.querySelector("#mp-form-description"),p=i.querySelector("#mp-word-count"),s=()=>{const d=l.value.trim(),m=d?d.split(/\s+/).filter(c=>c.length>0).length:0;p.textContent=m,p.style.color=m>=20?"#28a745":"#666",i.querySelector("#mp-description-error").style.display="none"};l.addEventListener("input",s),s(),i.addEventListener("click",d=>{d.target===i&&i.remove()}),i.querySelector("#mp-modal-cancel").addEventListener("click",()=>i.remove());const a=i.querySelector("#mp-modal-save");a.addEventListener("click",async()=>{const d=i.querySelector("#mp-form-name").value.trim(),m=i.querySelector("#mp-form-description").value.trim(),c=m?m.split(/\s+/).filter(u=>u.length>0).length:0;if(!d){alert("Project name is required");return}if(c<20){const u=i.querySelector("#mp-description-error");u&&(u.style.display="block"),i.querySelector("#mp-form-description").focus();return}const g=i.querySelector("#mp-form-showreel_link").value.trim();if(g&&!L(g)){const u=i.querySelector("#mp-showreel-error");u&&u.classList.add("visible"),i.querySelector("#mp-form-showreel_link").classList.add("error"),i.querySelector("#mp-form-showreel_link").focus();return}const n=i.querySelector("#mp-form-external_link").value.trim();if(n&&!k(n)){const u=i.querySelector("#mp-url-error");u&&u.classList.add("visible"),i.querySelector("#mp-form-external_link").classList.add("error"),i.querySelector("#mp-form-external_link").focus();return}a.disabled=!0,a.textContent="Saving...",i.style.display="none";try{const u=await R(async()=>await ee(r.id,{name:d,description:m,external_link:_(i.querySelector("#mp-form-external_link").value),showreel_link:_(i.querySelector("#mp-form-showreel_link").value),display_order:parseInt(i.querySelector("#mp-form-display_order").value)||0,categories:[...o],feature_image_url:t.feature_image_url||"",gallery_images:t.gallery_images||[]}),"Saving Changes");await $("project_update",{id:u.id,name:u.name});const b=f.findIndex(h=>h.id===r.id);b>-1&&(f[b]=u),f.sort((h,C)=>(h.display_order||0)-(C.display_order||0)),i.remove(),E(e)}catch(u){console.error("Error updating project:",u),i.style.display="",alert("Error updating project. Please try again."),a.disabled=!1,a.textContent="Save Changes"}})}function me(){return new Promise(r=>{if(window.$memberstackDom)r();else{const e=setInterval(()=>{window.$memberstackDom&&(clearInterval(e),r())},100)}})}function ue(){return new Promise((r,e)=>{if(window.supabase){r();return}let o=0;const t=50,i=setInterval(()=>{o++,window.supabase?(clearInterval(i),r()):o>=t&&(clearInterval(i),e(new Error("Supabase library not loaded")))},100)})}async function Y(){const r=document.querySelector(".supabase-project-container");if(!r){console.warn("Could not find .supabase-project-container");return}const e=document.createElement("style");e.textContent=W,document.head.appendChild(e);const o=document.createElement("div");o.className="mp-container",o.innerHTML='<div class="mp-loading">Loading projects...</div>',r.appendChild(o);try{if(await ue(),await me(),!J()){o.innerHTML='<div class="mp-loading">Error: Could not initialize Supabase</div>';return}const{data:t}=await window.$memberstackDom.getCurrentMember();if(!t){o.innerHTML='<div class="mp-loading">Please log in to view your projects.</div>';return}v=t,console.log("Current member:",v.id);const{data:i}=await y.from("members").select("profile_complete").eq("memberstack_id",t.id).single();q=(i==null?void 0:i.profile_complete)||!1,console.log("Profile complete:",q),x=await K(),console.log(`Categories loaded: ${x.directories.length} directories, ${x.subDirectories.length} sub-directories`),f=await Z(t.id),E(o)}catch(t){console.error("Error initializing member projects:",t),o.innerHTML='<div class="mp-loading">Error loading projects. Please refresh the page.</div>'}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Y):Y()})();
