(function(){const D="https://epszwomtxkpjegbjbixr.supabase.co",$="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVwc3p3b210eGtwamVnYmpiaXhyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAzMTE5MzUsImV4cCI6MjA4NTg4NzkzNX0.TJPI5NQmWHR6F5eGVZH26Mzj601RDp5bgcpYZFVymwQ";let p,u=[],b=[],g={},l=new Set;const q=`
    .directory-filters {
      margin-bottom: 32px;
    }
  ...
  `;function z(){if(!document.getElementById("directory-filter-styles")){const e=document.createElement("style");e.id="directory-filter-styles",e.textContent=q,document.head.appendChild(e)}}function L(){const r=window.location.pathname.match(/\/directories\/([^\/]+)/);return r?r[1]:null}async function _(){const e=L();if(!e){console.log("Directory filter: Not on a directory page");return}const r=document.querySelector(".x-member-grid-container")||document.querySelector(".directory-members-grid"),i=document.querySelector(".directory-filters");if(!r){console.log("Directory filter: No members grid container found (.x-member-grid-container or .directory-members-grid)");return}if(z(),!window.supabase){console.error("Directory filter: Supabase library not loaded");return}p=window.supabase.createClient(D,$),r.innerHTML='<div class="w-dyn-list"><div class="directory-loading">Loading members...</div></div>';try{await I(e),i&&F(i),x(r,b)}catch(o){console.error("Directory filter error:",o),r.innerHTML='<div class="directory-empty"><h3>Error loading members</h3><p>Please refresh the page.</p></div>'}}async function I(e){console.log("Directory filter: Loading data for",e);const{data:r,error:i}=await p.from("directories").select("id, name, slug").eq("slug",e).single();if(i||!r)throw new Error("Directory not found: "+e);console.log("Directory filter: Found directory",r.name,r.id);const{data:o,error:d}=await p.from("sub_directories").select("id, name, slug").eq("directory_id",r.id).order("name");if(d)throw d;u=o||[],console.log("Directory filter: Found",u.length,"sub-directories");const t=new Set;g={};const{data:c,error:m}=await p.from("member_directories").select("member_id").eq("directory_id",r.id);m?console.log("Directory filter: Error fetching parent directory members:",m):(console.log("Directory filter: Found",(c||[]).length,"members via parent directory"),(c||[]).forEach(n=>t.add(n.member_id)));const a=u.map(n=>n.id);if(a.length>0){const{data:n,error:w}=await p.from("member_sub_directories").select("member_id, sub_directory_id").in("sub_directory_id",a);w?console.log("Directory filter: Error fetching sub-directory members:",w):(console.log("Directory filter: Found",(n||[]).length,"member-subdirectory links"),(n||[]).forEach(f=>{t.add(f.member_id),g[f.member_id]||(g[f.member_id]=[]),g[f.member_id].push(f.sub_directory_id)}))}if(console.log("Directory filter: Found",t.size,"unique members total"),t.size===0){b=[];return}const{data:s,error:y}=await p.from("members").select(`
      ...
      `).in("id",Array.from(t)).not("memberstack_id","is",null).not("webflow_id","is",null).in("subscription_status",["active","trialing"]).order("name");if(y)throw y;console.log("Directory filter: Fetched",(s||[]).length,"members from database");const h={},v={};(s||[]).forEach(n=>{h[n.status||"null"]=(h[n.status||"null"]||0)+1,v[n.subscription_status||"null"]=(v[n.subscription_status||"null"]||0)+1}),console.log("Directory filter: Status breakdown:",h),console.log("Directory filter: Subscription status breakdown:",v),b=(s||[]).filter(n=>!(!n.memberstack_id||!n.webflow_id||n.subscription_status!=="active"&&n.subscription_status!=="trialing")).map(n=>({...n,subDirectoryIds:g[n.id]||[],subDirectoryNames:(g[n.id]||[]).map(w=>{var f;return(f=u.find(C=>C.id===w))==null?void 0:f.name}).filter(Boolean)})),console.log("Directory filter: Loaded",b.length,"members after filtering")}function F(e){var m,a;if(u.length===0){e.innerHTML="";return}const r='<svg class="checkmark" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>',i='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>',o=u.map(s=>`
    ...
    `,e.querySelectorAll(".directory-filter-desktop .directory-filter-btn").forEach(s=>{s.addEventListener("click",()=>A(s,e))}),(m=e.querySelector(".directory-filter-clear"))==null||m.addEventListener("click",()=>{S(e)});const t=e.querySelector(".directory-filter-mobile-trigger"),c=e.querySelector(".directory-filter-mobile-dropdown");t==null||t.addEventListener("click",()=>{const s=c.classList.contains("open");c.classList.toggle("open",!s),t.setAttribute("aria-expanded",!s)}),document.addEventListener("click",s=>{var y;(y=e.querySelector(".directory-filter-mobile"))!=null&&y.contains(s.target)||(c==null||c.classList.remove("open"),t==null||t.setAttribute("aria-expanded","false"))}),e.querySelectorAll(".directory-filter-mobile-item").forEach(s=>{s.addEventListener("click",()=>M(s,e))}),(a=e.querySelector(".directory-filter-mobile-clear"))==null||a.addEventListener("click",()=>{S(e),c==null||c.classList.remove("open"),t==null||t.setAttribute("aria-expanded","false")})}function M(e,r){const i=e.dataset.subId;l.has(i)?(l.delete(i),e.classList.remove("active")):(l.add(i),e.classList.add("active")),r.querySelectorAll(`.directory-filter-desktop .directory-filter-btn[data-sub-id="${i}"]`).forEach(a=>{a.classList.toggle("active",l.has(i))}),k(r);const o=r.querySelector(".directory-clear-item"),d=r.querySelector(".directory-filter-mobile-clear");o&&(o.style.display=l.size>0?"":"none"),d&&(d.style.display=l.size>0?"":"none");const t=E(),c=document.querySelector(".x-member-grid-container")||document.querySelector(".directory-members-grid");c&&x(c,t);const m=r.querySelector(".directory-member-count");if(m){const a=l.size>0?` (filtered from ${b.length})`:"";m.textContent=`${t.length} members${a}`}}function k(e){const r=e.querySelector(".directory-filter-mobile-label");if(r)if(l.size===0)r.textContent="Filter by specialty";else if(l.size===1){const i=u.find(o=>l.has(o.id));r.textContent=(i==null?void 0:i.name)||"1 filter selected"}else r.textContent=`${l.size} filters selected`}function A(e,r){const i=e.dataset.subId;l.has(i)?(l.delete(i),e.classList.remove("active")):(l.add(i),e.classList.add("active")),r.querySelectorAll(`.directory-filter-mobile-item[data-sub-id="${i}"]`).forEach(a=>{a.classList.toggle("active",l.has(i))}),k(r);const o=r.querySelector(".directory-clear-item"),d=r.querySelector(".directory-filter-mobile-clear");o&&(o.style.display=l.size>0?"":"none"),d&&(d.style.display=l.size>0?"":"none");const t=E(),c=document.querySelector(".x-member-grid-container")||document.querySelector(".directory-members-grid");c&&x(c,t);const m=r.querySelector(".directory-member-count");if(m){const a=l.size>0?` (filtered from ${b.length})`:"";m.textContent=`${t.length} members${a}`}}function S(e){l.clear(),e.querySelectorAll(".directory-filter-btn").forEach(t=>{t.classList.remove("active")}),e.querySelectorAll(".directory-filter-mobile-item").forEach(t=>{t.classList.remove("active")});const r=e.querySelector(".directory-clear-item"),i=e.querySelector(".directory-filter-mobile-clear");r&&(r.style.display="none"),i&&(i.style.display="none"),k(e);const o=document.querySelector(".x-member-grid-container")||document.querySelector(".directory-members-grid");o&&x(o,b);const d=e.querySelector(".directory-member-count");d&&(d.textContent=`${b.length} members`)}function E(){return l.size===0?b:b.filter(e=>e.subDirectoryIds.some(r=>l.has(r)))}function x(e,r){if(r.length===0){e.innerHTML=`
      ...
      `;return}const i=r.map(o=>{var y,h,v;const d=o.header_image_url||o.profile_image_url||"",t=o.profile_image_url||"",c=((y=o.suburbs)==null?void 0:y.name)||"",m=((h=o.suburbs)==null?void 0:h.slug)||"",a=((v=o.membership_types)==null?void 0:v.name)||"Professional",s=o.business_name||o.name;return`
        <div role="listitem" class="w-dyn-item">
          <div class="x-member-card">
            <div class="x-member-feature-image-container">
              <div class="x-member-type hide">
                ...
                <div class="text-style-allcaps text-color-white small">${a}</div>
                <div>${a}</div>
              </div>
